# SeekDB Node SDK vs Python SDK 功能对齐清单

本文档用于跟踪 Node SDK 与 Python SDK 之间的功能对齐情况。

## 📊 功能对齐方法论

### 1. 信息源优先级（按顺序）

1. **API.md** - 官方 API 参考文档（最权威）
2. **README.md Table of Contents** - 功能概览和结构
3. **测试文件列表** - 实际功能的测试覆盖
4. **源代码结构** - 实际实现的功能
5. **示例代码** - 实际使用场景
...
- **Level 3: Collection 对象（数据操作）**
- **Level 4: 辅助功能（EmbeddingFunction、Filters 等）**
...
### Phase 1: P0 功能（AdminClient）

1. ✅ 创建 `SeekDBAdminClient` 类
2. ✅ 实现 `create_database()`
3. ✅ 实现 `get_database()`
4. ✅ 实现 `delete_database()`
5. ✅ 实现 `list_databases()`
6. ✅ 创建 `Database` 类型定义
7. ✅ 编写测试用例
...
1. ⬜ 实现 `DefaultEmbeddingFunction`
2. ⬜ 实现 `Database` 对象完整功能

### Phase 3: P2 功能

1. ✅ 实现 `collection.describe()`
2. ✅ 实现 `collection.peek()`
3. ✅ 实现 `$regex` 过滤器
...
1. **每周检查**: 查看 pyseekdb 的 CHANGELOG 或 git commits
2. **新功能检测**: 监控 pyseekdb 新增的 API
3. **测试同步**: 确保测试用例覆盖率与 Python SDK 一致
4. **文档同步**: README 和 API 文档保持同步

---

## 📚 参考资源

- Python SDK API 文档: `pyseekdb/API.md`
