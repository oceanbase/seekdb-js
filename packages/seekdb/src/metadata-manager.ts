...
const METADATA_TABLE_NAME = "sdk_collections";
 ...
 * Should be called after any DML operation (insert/update/delete) on the collection data table
 */
export async function updateCollectionTimestamp(
  client: InternalClient,
  collectionName: string,
): Promise<void> {
  const updateSql = `
  ...
  try {
    await client.execute(updateSql, [collectionName]);
  } catch (error) {
    // If table doesn't exist, silently ignore (v1 collection)
    if (
      error instanceof Error &&
