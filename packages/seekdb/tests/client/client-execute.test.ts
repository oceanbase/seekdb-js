/**
 * Server mode: validates client.execute in various scenarios:
 * - SELECT (no params / with params)
 * - DDL (CREATE TABLE, DROP TABLE)
 * - DML (INSERT, UPDATE, DELETE) with params
 * - SET user variable and session state
 * - Hybrid search on SQL-created table (when server supports it)
 */
...
const TABLE_HYBRID = "doc_table_search_sql_srv";

describe("Server Mode - client.execute", () => {
  ...
  test("execute SET user variable and SELECT uses session state", async () => {
    ...
    const rows = await client.execute("SELECT @exec_var_srv AS v");
    expect(rows).toBeDefined();
    expect(Array.isArray(rows)).toBe(true);
  ...
  test("hybrid search on table created by SQL (no collection API) returns rows", async () => {
    ...
    const searchSql = `SELECT DBMS_HYBRID_SEARCH.SEARCH('${TABLE_HYBRID.replace(/'/g, "''")}', @search_parm_srv) as result`;
