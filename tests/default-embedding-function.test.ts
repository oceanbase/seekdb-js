...
describe("Default Embedding Function Tests", () => {
  ...
  test("server mode default embedding function", async () => {
    ...
    // Create collection without providing embeddingFunction
    ...
    try {
      ...
      console.log(`   Added ${testDocuments.length} documents (vectors auto-generated)`);
      ...
      if (results.embeddings && results.embeddings[0]) {
        ...
        console.log(`   Verified: document and embedding (dim=${results.embeddings[0].length}) stored correctly`);
      } else {
        console.log(`   Verified: document stored correctly (embedding not included in get results)`);
      }
      ...
      try {
        ...
        console.log(`   Found ${fulltextResults.ids.length} results from full-text search`);
      } catch (e) {
      ...
      try {
        ...
        console.log(`   Found ${combinedResults.ids.length} results from hybrid search`);
      } catch (e) {
