...
async function main() {
  ...
  console.log('='.repeat(100));
  console.log("Goal: Find documents similar to 'AI research' AND containing 'machine learning'\n");
  ...
  try {
    ...
    console.log('Note: hybridSearch() is not supported on this database version');
  }

  console.log('\n' + '='.repeat(100));
  console.log('SCENARIO 2: Independent Filters for Different Search Types');
  console.log('='.repeat(100));
  console.log("Goal: Full-text='neural' (year=2024) + Vector='deep learning' (popularity>=90)\n");
  ...
  try {
    const hybridResult2 = await collection.hybridSearch({
      query: { whereDocument: { $contains: 'neural' }, where: { year: { $eq: 2024 } }, nResults: 10 },
      knn: { queryTexts: ['deep learning'], where: { popularity: { $gte: 90 } }, nResults: 10 },
      rank: { rrf: {} },
      nResults: 5,
    });
